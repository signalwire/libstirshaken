...
static void stirshaken_usage(const char *name)
{
    ...
    fprintf(stderr, "\t\t %s			: decode JWT and verify signature against public key given to --%s\n", COMMAND_NAME_JWT_KEY_CHECK, OPTION_NAME_PUBKEY);
    fprintf(stderr, "\t\t %s			: decode JWT, download referenced certificate and verify signature against public key from that cert\n", COMMAND_NAME_JWT_CHECK);
