...
stir_shaken_status_t stir_shaken_unit_test_jwt_verify_with_cert(void)
{
    ...
    printf("Created SIP Identity Header\n\n");
    printf("SIP Identity Header:\n%s\n\n", sih);
	r = stir_shaken_jwt_passport_dump_str(&passport, 1);
	stir_shaken_assert(r != NULL, "Failed to dump PASSporT");
...
int main(void)
{
	...
	if (stir_shaken_unit_test_jwt_verify_with_cert() != STIR_SHAKEN_STATUS_OK) {
