...
stir_shaken_status_t stir_shaken_unit_test_as_authenticate_to_sih(void)
{
    stir_shaken_passport_t *passport = NULL;
    char *p = NULL, *s = NULL, *encoded = NULL, *sih = NULL;
	...
	sprintf(private_key_name, "%s%c%s", path, '/', "u16_private_key.pem");
	sprintf(public_key_name, "%s%c%s", path, '/', "u16_public_key.pem");

    printf("=== Unit testing: STIR/Shaken AS authenticate to SIP Identity Header\n\n");
...
int main(void)
{
	...
	if (stir_shaken_unit_test_as_authenticate_to_sih() != STIR_SHAKEN_STATUS_OK) {
